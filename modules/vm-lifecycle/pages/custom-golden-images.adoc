...
Create a base Fedora VM that we will customize:
...
xref:attachment$custom-golden-images/golden-image-base-vm.yaml[Download golden-image-base-vm.yaml]
...
Now we will create a DataSource that references the base VM's disk. This DataSource will be used to deploy new VMs.

Create the DataSource:

xref:attachment$custom-golden-images/golden-image-datasource.yaml[Download golden-image-datasource.yaml]
...
xref:attachment$custom-golden-images/webserver-vm-from-golden.yaml[Download webserver-vm-from-golden.yaml]
...
xref:attachment$custom-golden-images/webserver-vm-2.yaml[Download webserver-vm-2.yaml]
...
. Deploy a VM from the current golden image
. Apply updates or changes
. Generalize the VM
. Create a new DataSource with version in the name (e.g., `fedora-nginx-golden-v2`)
...
If you encounter issues during any step of this tutorial, refer to the xref:golden-images-troubleshooting.adoc[Golden Images Troubleshooting Guide] for detailed verification commands, common issues, and solutions.
