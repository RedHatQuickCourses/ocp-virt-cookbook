...
└─ openshift-cnv Namespace
   └─ HyperConverged CR (single entry point)
      ├─ KubeVirt Operator
      │    ├─ virt-api        — API server & console access
      │    └─ virt-controller — VMI lifecycle & scheduling
      │         ├─ Worker Node 1
      ...
      │                   ├─ libvirtd (session mode)
      │                   └─ QEMU/KVM
      ├─ CDI Operator        — VM image import/upload/clone
      ├─ SSP Operator        — templates & boot sources
...
TIP: For a broader architecture overview including VM execution flows, storage and networking models, and comparisons with traditional hypervisors, see xref:architecture-overview.adoc[].
