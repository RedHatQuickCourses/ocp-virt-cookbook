...
* *`metadata.name`*: Kubernetes name of the `NetworkAttachmentDefinition`.
...
Once the file is saved as `lnbr0-bridge.net-attach-def.yaml` (where `net-attach-def` is the valid Kubernetes short-name), go ahead and apply the `network-attachment-definition` to the OpenShift cluster:
...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/{ocp-docs-version}/html/virtualization/networking#virt-connecting-vm-to-linux-bridge_virt-connecting-vm-to-linux-bridge[OpenShift Virtualization - Connecting a VM to a Linux bridge,window=_blank]
