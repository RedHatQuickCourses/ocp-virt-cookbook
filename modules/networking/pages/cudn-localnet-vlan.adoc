...
Create namespaces for VMs that will use the localnet secondary network with VLAN configuration. ClusterUserDefinedNetwork (CUDN) allows the same network to be available across multiple namespaces.
...
For OVS bridge and bridge mapping troubleshooting, including CUDN-specific verification steps, see the xref:ovs-bridge-troubleshooting.adoc#_clusteruserdefinednetwork_cudn_verification[OVS Bridge Verification Troubleshooting Guide].
...
* How to create a custom OVS bridge on an extra physical NIC and configure bridge mappings for VLAN traffic
* How to create a `ClusterUserDefinedNetwork` with VLAN tagging that spans multiple namespaces
* How to deploy VMs with static IP addresses via cloud-init on the CUDN VLAN secondary network
* How to verify cross-namespace connectivity between VMs sharing the same CUDN VLAN network
* The differences between CUDN and NetworkAttachmentDefinition approaches for VLAN-based networks
