...
xref:udn-primary-networks.adoc[User Defined Networks (Layer 2)]::
...
xref:cudn-localnet-vlan.adoc[Cluster UDN with VLANs]::
Advanced cluster-wide UDN configuration with VLAN support.

xref:linux-bridges.adoc[Linux Bridges]::
Traditional Linux bridge networking for VMs.

xref:ovn-overlay-cloud.adoc[OVN Overlay on Cloud]::
Using OVN-Kubernetes overlay networks in cloud environments.
...
* xref:vm-configuration:index.adoc[Configure VMs] with cloud-init
