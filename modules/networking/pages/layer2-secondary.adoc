...
Apply the `NetworkAttachmentDefinition` into the pre-defined namespace `example-l2-network` on the OpenShift cluster using `oc apply`:
...
Note that the only changes from the firt VM are the name, IP address and root volume name.
