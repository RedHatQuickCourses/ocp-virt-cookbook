...
IMPORTANT: The namespace must be created and labeled before any workload deployment happens and cannot be changed once created.
...
You have successfully configured a Layer 2 primary network using User Defined Networks in OpenShift Virtualization. This setup provides:

* Custom IP addressing (192.168.100.0/24) for the namespace
* Complete network isolation from other namespaces
* Persistent IPAM lifecycle
* LoadBalancer service exposure for external access

== See Also

* xref:localnet-secondary.adoc[Localnet Secondary Networks with CUDN]
* xref:vm-configuration:cloud-init-basics.adoc[Cloud-init Configuration]
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/multiple_networks/understanding-multiple-networks[OpenShift Multiple Networks,window=_blank]
