...
* How OVS bridges differ from Linux bridges and why they do not appear in `ip addr show` output
* How to verify OVS bridge creation, physical interface attachment, and bridge port configuration using `ovs-vsctl`
* How to verify OVN bridge mappings that connect logical network names to physical OVS bridges
* How to troubleshoot common issues including missing bridges, interface attachment failures, and logical name mismatches
* How to verify ClusterUserDefinedNetwork (CUDN) resources, auto-created NADs, and cross-namespace network availability
