...
IMPORTANT: This differs from traditional `eth0`, `eth1` naming. Always use `enpXsY` format in cloud-init network configurations.
...
TIP: When to use: Development environments, simple deployments, when DHCP is sufficient.
...
NOTE: Prerequisites: Ensure you have a secondary network configured (e.g., localnet VLAN 100).
