...
[source,bash,role=execute]
----
for vm in vm-burstable vm-guaranteed vm-besteffort; do
  pod=$(oc get vmi $vm -n resource-qos-demo -o jsonpath='{.metadata.name}')
  qos=$(oc get pod -n resource-qos-demo -l kubevirt.io/domain=$vm -o jsonpath='{.items[0].status.qosClass}')
  echo "$vm: QoS=$qos"
done
----
...
Expected output:
----
vm-burstable: QoS=Burstable
vm-guaranteed: QoS=Guaranteed
vm-besteffort: QoS=Burstable
----
...
for vm in vm-burstable vm-guaranteed vm-besteffort; do
  echo "=== $vm ==="
  oc get pod -n resource-qos-demo -l kubevirt.io/domain=$vm \
    -o jsonpath='{range .items[0].spec.containers[*]}{.name}: requests={.resources.requests} limits={.resources.limits}{"\n"}{end}'
  echo ""
done
----
