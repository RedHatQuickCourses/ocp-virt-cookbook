...
Install the test tools (the previous installation was lost with the reboot if the disk is ephemeral):
...
Run cyclictest with the same parameters as Step 8:
...
IMPORTANT: For production real-time workloads, build a custom container disk image with the RT kernel pre-installed rather than installing it at runtime. This eliminates the need for a post-boot kernel installation and reboot cycle. A container disk with the RT kernel also guarantees consistent, version-locked deployments across all RT VMs.
